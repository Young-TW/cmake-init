...
pub fn cmakelists_txt(project_name: &str, cxx_std: i32, mode: Option<&str>) {
    ...
    let origin_content = match mode {
        Some("CUDA") => include_str!("../files/cuda/CMakeLists.txt"),
        Some("HIP") => include_str!("../files/hip/CMakeLists.txt"),
        Some("MPI") => include_str!("../files/mpi/CMakeLists.txt"),
        _ => include_str!("../files/CMakeLists.txt"),
    };
